...
class LaneCandidatePublisher:
    ...
    def point_cloud_callback(self, msg):
        ...
        # 차선 감지
        _, right_points = self.extract_road_line(points)

        # 차선 부드럽게 만들기
        right_points = self.smooth_line(right_points)

        # 오른쪽 차선을 기준으로 오프셋 적용하여 중앙선 생성
        center_points = self.extract_offset_line(right_points, 1)

        # 1미터 예측 차선 생성
        predicted_center_points = self.predict_center_line(center_points, 2)

        # 감지된 차선 시각화
        self.publish_lane_marker([], right_points, msg.header)
    ...
    def extract_offset_line(self, right_points, offset):
        # 오른쪽 차선에 일정 오프셋을 적용하여 중앙선 생성
        offset_points = [[p[0], p[1] + offset, p[2]] for p in right_points]
